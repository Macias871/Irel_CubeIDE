		  ...
		  rx_queue_status = xQueueReceive(Rs485_serial_rx_queue, &queue_first_byte, 1);
			...
			else
			{
			  ...
			  if((USART1->SR & USART_SR_RXNE) == 0)
				...
				queue_status = xQueueReceive(Rs485_serial_tx_queue, &queue_first_byte_tx, 5);
...
void Rs485_received_execute(char *cmd)
{
	...
	memcpy(buffer_rs485_rx,&cmd[1],17);
	crc_rx = calc_crc(buffer_rs485_rx,12);

	//memset(chars,0,100);
	//chars[0] = datahex(buffer_rs485_rx[0]);
	hexStr2Arr(&chars[0],&buffer_rs485_rx[0],8);

	RX_Frame.crc = (chars[6] << 8) | (chars[7]);
...
void serial_send_string(char *str)
{
    ...
    for (i = 0; ; i++) {
        ...
        if ((str[i] == '\0')) break;
        xQueueSend(Rs485_serial_tx_queue, (void*)&str[i], ( TickType_t ) 5);
