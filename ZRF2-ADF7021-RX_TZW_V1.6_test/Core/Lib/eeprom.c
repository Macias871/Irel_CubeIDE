...
EEPROM_Status EEPROM_Write(uint32_t Address, uint16_t Data) {
	...
	if (status == EEPROM_COMPLETE) {
		...
		*(volatile uint32_t *)Address = (uint16_t)Data; // Program the new data
...
void ReadBuffer_EEPROM(uint32_t addr, volatile uint16_t *buffer, uint16_t len) {
	...
	for (i = 0; i < len; i++) *buffer++ = EEPROM_Read(addr + i);
}
...
void writeToEEPROM (uint32_t address, uint32_t value)
    {
      ...
      while (flash_ok != HAL_OK)
      {
        flash_ok = HAL_FLASHEx_DATAEEPROM_Erase (FLASH_TYPEERASEDATA_BYTE, address);
      }
      flash_ok = HAL_ERROR;
      while (flash_ok != HAL_OK)
      {
        flash_ok = HAL_FLASHEx_DATAEEPROM_Program (FLASH_TYPEPROGRAMDATA_BYTE, address, value);
      }
      flash_ok = HAL_ERROR;
      while (flash_ok != HAL_OK)
      {
